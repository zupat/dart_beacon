...
class CurrencyController with BeaconController {
  ...
  late final animationOffsets = B.writable((0.0, 0.0));
  Timer? _timer;
  void startAnimation() {
    _timer?.cancel();
    // Start from both ends: left currency from -1.0, right currency from 1.0
    animationOffsets.value = (-1.0, 1.0);
    var elapsed = 0;
    const duration = 300;
    const interval = 16;
    _timer = Timer.periodic(const Duration(milliseconds: interval), (timer) {
      ...
      animationOffsets.value = (leftOffset, rightOffset);
      if (progress >= 1) {
        timer.cancel();
        animationOffsets.value = (0.0, 0.0);
      }
    });
  }
